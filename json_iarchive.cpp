...
#include <cstring>
json_iarchive::json_iarchive(Json::Value& root): _stack(root) {
	_clearIds();
	_loadClassMap();
}
...
void json_iarchive::load_override(object_id_type& t, int) {
	...
	if(!js.isIntegral()) {
		std::cout << cs_ObjectIdType << " is not found" << std::endl;
	...
	std::cout << "ObjectID: " << *_objectId << std::endl;
...
void json_iarchive::load_override(object_reference_type& t, int) {
	...
	if(!js.isIntegral()) {
		std::cout << cs_ObjectReferenceType << " is not found" << std::endl;
		return;
	}
	auto val = js.asInt();
	_objectId = val;
	_objectType = ObjectIDType::Reference;
// 	std::cout << "ObjectRefID: " << *_objectId << std::endl;
...
void json_iarchive::load_override(class_id_type& t, int) {
	...
	if(!js.isIntegral()) {
		std::cout << cs_ClassIdType << " is not found" << std::endl;
...
// 	std::cout << "ClassId: " << *_classId << std::endl;
	t = class_id_type(int(*_classId));
}
void json_iarchive::load_override(class_id_optional_type& t, int) {
	auto& js = _stack.getEnt()[cs_ClassIdOptionalType];
	if(!js.isIntegral()) {
		std::cout << cs_ClassIdOptionalType << " is not found" << std::endl;
		return;
	}
	auto val = js.asInt();
	_classId = val;
...
void json_iarchive::load_override(class_id_reference_type& t, int) {
	...
	if(!js.isIntegral()) {
		std::cout << cs_ClassIdReferenceType << " is not found" << std::endl;
		return;
	}
	auto val = js.asInt();
	_classId = val;
	...
	_objectId = boost::none;
// 	std::cout << "ClassRefId: " << *_classId << std::endl;
